 ...
 * YOY函数节点
 * <p>
 * 专门处理YOY函数的表达式节点，YOY是一个自定义函数，用于计算同比增长。
 * YOY = (当前值 - 去年同期值) / 去年同期值
 * <p>
...
public class YoyNode extends FunctionNode {

	private static final Long YOY_FUNCTION_ID = 51L;
	private static final String YOY_FUNCTION_NAME = "YOY";
	...
	protected ExecutionResult doExecute(ExecutionContext context) {
		...
		String resultTable = calculateYoyByTimeShift(currentTable, previousTable, context);
	...
	/**
	 * 将时间过滤条件整体往前推一年，支持 EQ（单值）和 IN（多值）两种形式。
	 */
		...
		for (FilterCondition filter : originalFilters) {
			...
			if (isPeriodEqFilter(filter)) {
				String shiftedPeriod = TimeCompareUtils.compare(
						String.valueOf(filter.getValue()), granularity, Constants.CompareType.YOY);
		...
		if (value instanceof Collection) {
			for (Object v : (Collection<?>) value) {
				result.add(TimeCompareUtils.compare(
						String.valueOf(v), granularity, Constants.CompareType.YOY));
			}
			...
			// 降级：当作单值处理
			result.add(TimeCompareUtils.compare(
					String.valueOf(value), granularity, Constants.CompareType.YOY));
		}
		return result;
	}
	...
	private String calculateYoyByTimeShift(DuckDBTableResult currentTable,
										   DuckDBTableResult previousTable,
										   ExecutionContext context) {
		TableStructure structure = analyzeTableStructure(currentTable, context);
		String resultTable = context.generateTempTableName("yoy_result");
		String sql = buildYoySQL(currentTable.getTableName(), previousTable.getTableName(), resultTable, structure);
		log.debug("YOY SQL (time-shift): {}", sql);
		DuckDBOperator.executeUpdate(sql);
		return resultTable;
	}

	private String buildYoySQL(String currentTable,
							   String previousTable,
							   String resultTable,
							   TableStructure structure) {
		return "CREATE TABLE " + resultTable + " AS "
				+ "SELECT " + buildYoySelectClause(structure)
	...
	/**
	 * SELECT 子句：维度列 + 时间列（取 cur） + YOY 计算结果
	 */
	private String buildYoySelectClause(TableStructure structure) {
		...
		select.append(buildYoyCalculation(
	 ...
	 * JOIN ON 条件：
	 * - 维度列等值匹配
	 * - 时间列：将 cur 的时间偏移一年后与 prev 的时间对齐，支持多时间点同时计算同比
	 *   例：cur.period='202506' -> shift -> '202406' = prev.period
	 */
	...
	/**
	 * 构建"将当期时间往前推一年"的 DuckDB SQL 表达式。
	 * 时间列格式固定为 yyyyMM，如 202506。
	 *
	...
	/**
	 * YOY 计算的 CASE WHEN 表达式
	 */
	private String buildYoyCalculation(String currentValueExpr, String previousValueExpr) {
		...
		switch (calculationType) {
			...
			case RATIO:
				...
				break;
			default:
				throw new UnsupportedOperationException("Unsupported YOY calculation type: " + calculationType);
		}
	...
	private TableStructure analyzeTableStructure(DuckDBTableResult table, ExecutionContext context) {
		...
		for (String col : allColumns) {
			if (!col.equals(structure.getTimeColumn())
					&& !col.equals(structure.getMeasureColumn())
					&& !col.equals("metric_code")) {
	...
	private String identifyTimeColumn(List<String> columns) {
		...
		throw new IllegalStateException("Cannot identify time column in table. Available columns: " + columns);
	}
	...
	private DuckDBTableResult toDuckDBTable(ExecutionResult result, ExecutionContext context) {
		...
		throw new UnsupportedOperationException(
				"Only DuckDBTableResult supported for YOY calculation, got: " + result.getType());
	}
